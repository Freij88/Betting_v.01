...
async function ingestCSV(filePath: string, league: string) {
...
async function updateTeamStats(teamName: string, sport: string, goalsFor: number, goalsAgainst: number, result: string) {
    ...
    if (stats) {
        const newForm = (stats.form as string[] || []);
